...
if (~isfield(noise_params,'darkcount'))||isempty(noise_params.darkcount)   % Default PMT dark count rate (and excess photon rate from light leakage and autofluoresence)
...
if (~isfield(noise_params,'bleedp'))||isempty(noise_params.bleedp)         % Default electronics pixel bleed through probability is 0.3
    noise_params.bleedp = 0.3;
end
if (~isfield(noise_params,'bleedw'))||isempty(noise_params.bleedw)         % Default electronics average pixel bleed-through (if it occurs) is 0.4
    noise_params.bleedw = 0.4;
end
if (~isfield(noise_params,'bleedp'))||isempty(noise_params.bleedp)      
    noise_params.bleedp = 0.2;
end
if (~isfield(noise_params,'bleedw'))||isempty(noise_params.bleedw)      
