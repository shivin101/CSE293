function opt_out = simulate_optical_propagation(vol_params,psf_params,vol_out)
...
if ~isstruct(Uin)
  if(psf_params.fastmask)
    ...
    [mask,~,~,psfTM,psfBM] = genCorticalLightPathLite(vol_params,...
    ...
    [~,psf,~,psfT,psfB] = genCorticalLightPathLite(vol_params,psf_params, ...
    ...
    [mask,psf,~,psfT,psfB] = genCorticalLightPathLite(vol_params,psf_params, ...
                                        neur_ves_A,neur_ves_B,neur_ves_C,Uin);        % Generate diffraction-limited tissue obscuration mask
    if(psf_params.sampling>max(size(mask)))
      mask       = ones(size(mask),'like',mask);
    end
