function [opt_out,psfs] = simulate_optical_propagation2(vol_params,psf_params,vol_out,scattvol)
...
if (isfield(vol_out,'neur_ves_all'))&&~isempty(vol_out.neur_ves_all)
  ...
  if(~all(nvpx==vcpx))
    ...
    elseif(all(nvpx>=vcpx))
      ...
      if(vcpx(3)>=nvpx(3))
        TMPvasc(1+floor(dpx(1)):end-ceil(dpx(1)),1+floor(dpx(2)):end-ceil(dpx(2)), ... 
          1:end-2*dpx(3)) = vol_out.neur_ves_all;        
      else
        TMPvasc(1+floor(dpx(1)):end-ceil(dpx(1)),1+floor(dpx(2)):end-ceil(dpx(2)),:) ...
...
if (isfield(psf_params,'scatter_sz'))&&(~isempty(psf_params.scatter_sz))
    ...
    if(rem(zCsz(3),1)~=0)
      neur_ves_C2(:,:,end) = neur_ves_C2(:,:,end)*rem(zCsz(3),1);
    end
