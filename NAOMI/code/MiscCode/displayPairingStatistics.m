...
TMPsp1 = intersect(unique(pcaica.pairs(pcaica.corrvals>0.5,1)), unique(suite2p.pairs(suite2p.corrvals>0.5,1)));
...
fprintf('\t - Only CNMF (>0.1,>0.3,>0.5): %d, %d, %d\n', ...
    ...
    numel(setdiff(unique(cnmf.pairs(cnmf.corrvals>0.4,1)),union(TMPcs3,TMPcp3))), ...
