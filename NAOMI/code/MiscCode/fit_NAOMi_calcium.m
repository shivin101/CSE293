...
if strcmp(optType, 'BayesOpt')
    ...
    cal_fun2 = @(z) calciumFun(spike_time, ...
                            t_frame(1:length(fmean_norm)), fmean_norm,...
                                   max(t_frame(1:length(fmean_norm))), z); % Set up function for fmincon optimization
...
function out_val = calciumFun(spike_time, t_frame, fmean_norm, t_ephys, params)
...
[~,~,simFluor]  = calcium_dynamics(spikes2, cal_params, 'GCaMP6');         % Simulate the fluorescence time-traces
