...
if (~isfield(vol_params,'vol_sz'))||isempty(vol_params.vol_sz)
    vol_params.vol_sz = [100,100,50];                                      % Default Volume size is 100x100x30um
end
...
if (~isfield(vol_params,'min_dist'))||isempty(vol_params.min_dist)         % Default minimum distance between neuron centers (Default = 1.5)
    vol_params.min_dist = 16;
end
if (~isfield(vol_params,'vres'))||isempty(vol_params.vres)                 % Default volume resolution(Default 10 points per unit)
    vol_params.vres = 2;
end
...
if (~isfield(vol_params,'N_bg'))||isempty(vol_params.N_bg)                 % Default number of background processes
    vol_params.N_bg = 1e6;
end
if (~isfield(vol_params,'vol_depth'))||isempty(vol_params.vol_depth)       % Default depth of the volume within the tissue is 100um
    vol_params.vol_depth = 200;
end
if (~isfield(vol_params,'dendrite_tau'))||isempty(vol_params.dendrite_tau) % Default dendrite decay strength exponential distance is 5
    vol_params.dendrite_tau = 5;
end
if (~isfield(vol_params,'verbose'))||isempty(vol_params.verbose)           % Default verbose level is 1
    vol_params.verbose = 1;
end
